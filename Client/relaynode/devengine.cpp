...
void DevEngine::Run()
{
	...
	{
		m_pnsProxy->OnTimer();
		DevRepo::Instance()->OnTimer();
		SessionRepo::Instance()->OnTimer();
...
bool DevEngine::InitP2PLib()
{
	...
	uint32_t ret = m_p2pAgent->Start(0, 9999);
	if (ret != P2P_OK)
	{
...
void DevEngine::HandlePeerData(void *buff, int size, const devid_t &fromDev)
{
	...
	pdevice = DevRepo::Instance()->GetDevice(fromDev);
	...
	pdevice->HandlePeerData(buff, size);
}
...
void DevEngine::OnConnEvent(event_type_t type, const devid_t &devid)
{
	switch (type)
	{
		case p2p_connected:
			...
			DevRepo::Instance()->AddDevice(devid);
			break;
		case p2p_disconnected:
			std::cout << "Device " << devid << " disconnected" << endl;
			DevRepo::Instance()->RmDevice(devid);
			break;
		case p2p_connecting:
