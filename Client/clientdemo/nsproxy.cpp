...
//	std::cout << "NsProxy::HandleNsData" << endl;
...
void NsProxy::HandleStartRealPlay(NSMessageReader& msgReader)
{
    ...
    if (fParseOk)
        {
            DevEngine::Instance()->P2PAgent()->StartSessionRelay(sessionId.c_str(), url.c_str(), NULL);
        }
}
...
void NsProxy::HandleStopRealPlay(NSMessageReader& msgReader)
{
    ...
    if (fParseOk)
        {
            DevEngine::Instance()->P2PAgent()->StopSessionRelay(sessionId.c_str());
        }
}
...
bool NsProxy::SendRegisterReq()
{
	...
	protocalBuilder.AddDataPram("support", "Play");
